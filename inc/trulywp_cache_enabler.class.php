...
final class TrulyWP_Cache_Enabler {
    ...
    */
    ...
    public function __construct()
    {
        ...
        if ( is_admin() ) {
            ...
            add_filter(
                'plugin_action_links_' .TRULYWP_CE_BASE,
                array(
                    __CLASS__,
                    'action_links'
                )
    ...
    */
    ...
    public static function on_activation() {
        ...
        copy(TRULYWP_CE_DIR . '/advanced-cache.php', WP_CONTENT_DIR . '/advanced-cache.php');
    }
    ...
    public static function on_upgrade_hook( $obj, $options ) {
        ...
        if ( $options['action'] == 'update' && $options['type'] == 'plugin' && array_key_exists('plugins', $options) ) {
            foreach ( (array)$options['plugins'] as $each_plugin ) {
                if ( preg_match("/^trulywp-cache\//", $each_plugin) ) {
                    // we got updated!
                    self::on_upgrade();
                }
            }
    ...
    */
    ...
    public static function install_later($id) {
        ...
        if ( ! is_plugin_active_for_network(TRULYWP_CE_BASE) ) {
            return;
        }
    ...
    */
    ...
    private static function _set_default_vars() {
        ...
        // disk cache
        if ( TrulyWP_Cache_Enabler_Disk::is_permalink() ) {
            self::$disk = new TrulyWP_Cache_Enabler_Disk;
        }
    }
    ...
    */
    ...
    public static function warning_is_permalink() {
        ...
        if ( !TrulyWP_Cache_Enabler_Disk::is_permalink() AND current_user_can('manage_options') ) { ?>

            <div class="error">
                <p><?php printf( __('The <b>%s</b> plugin requires a custom permalink structure to start caching properly. Please go to <a href="%s">Permalink</a> to enable it.', 'trulywp-cache'), 'TrulyWP Cache', admin_url( 'options-permalink.php' ) ); ?></p>
            </div>
    ...
    */
    ...
    public static function row_meta($input, $page) {
        ...
        return array_merge(
            $input,
            array(
                '<a href="https://trulywp.com/support/wordpress-trulywp-cache-plugin/" target="_blank">Support Page</a>',
            )
        );
    }
    ...
    */
    ...
    public static function add_dashboard_count( $items = array() ) {
        ...
        $items[] = sprintf(
            ...
            esc_html__('Disk Cache', 'trulywp-cache'),
            ( empty($size) ? esc_html__('Empty', 'trulywp-cache') : size_format($size) ),
            esc_html__('Cache Size', 'trulywp-cache')
        );
    ...
    */
    ...
    public static function get_cache_size() {
        ...
        if ( ! $size = get_transient('cache_size') ) {
            ...
            $size = (int) self::$disk->cache_size(TRULYWP_CE_CACHE_DIR);
    ...
    public static function add_admin_links($wp_admin_bar) {
        ...
        $wp_admin_bar->add_menu(
            array(
                ...
                'title'     => '<span class="ab-item">'.esc_html__('Clear Cache', 'trulywp-cache').'</span>',
                'meta'   => array( 'title' => esc_html__('Clear Cache', 'trulywp-cache') )
            )
        );
        ...
        if ( ! is_admin() ) {
            ...
            $wp_admin_bar->add_menu(
                array(
                    ...
                    'parent' => 'top-secondary',
                    'title'     => '<span class="ab-item">'.esc_html__('Clear URL Cache', 'trulywp-cache').'</span>',
                    'meta'   => array( 'title' => esc_html__('Clear URL Cache', 'trulywp-cache') )
                )
            );
        }
    }
    ...
    */
    ...
    public static function process_clear_request($data) {
        ...
        if ( is_multisite() && is_plugin_active_for_network(TRULYWP_CE_BASE) ) {
    ...
    */
    ...
    public static function clear_notice() {
        ...
        echo sprintf(
            ...
            esc_html__('The cache has been cleared.', 'trulywp-cache')
        );
    }
    ...
    */
    ...
    public static function publish_post_types($post_ID, $post) {
        ...
        // validate nonce
        if ( ! isset($_POST['_cache__status_nonce_' .$post_ID]) OR ! wp_verify_nonce($_POST['_cache__status_nonce_' .$post_ID], TRULYWP_CE_BASE) ) {
            return;
        }
    ...
    */
    ...
    public static function check_future_posts() {
        ...
        if ( $future_posts->have_posts() ) {
            ...
            sort($post_dates);
            TrulyWP_Cache_Enabler_Disk::record_advcache_settings(array(
                "cache_timeout" => strtotime($post_dates[0])));
        } else {
            TrulyWP_Cache_Enabler_Disk::delete_advcache_settings(array("cache_timeout"));
        }
    }
    ...
    */
    ...
    public static function clear_total_cache() {
        ...
        // clear disk cache
        TrulyWP_Cache_Enabler_Disk::clear_cache();
    ...
    */
    ...
    public static function handle_cache() {
        ...
        if ( empty($cached) ) {
            ob_start('TrulyWP_Cache_Enabler::set_cache');
            return;
        }
    ...
    */
    ...
    public static function add_clear_dropdown() {
        ...
        // validate nonce
        wp_nonce_field(TRULYWP_CE_BASE, '_cache__status_nonce_' .$GLOBALS['post']->ID);
        ...
        $available_options = array(
            esc_html__('Completely', 'trulywp-cache'),
            esc_html__('Page specific', 'trulywp-cache')
        );
        ...
        echo sprintf(
            ...
            esc_html__('Clear cache', 'trulywp-cache'),
    ...
    */
    ...
    public static function add_admin_resources($hook) {
        ...
        // plugin data
        $plugin_data = get_plugin_data(TRULYWP_CE_FILE);
        ...
        switch($hook) {
            ...
            case 'post.php':
                wp_enqueue_script(
                    'cache-post',
                    plugins_url('js/post.js', TRULYWP_CE_FILE),
                    array('jquery'),
                    $plugin_data['Version'],
                    true
                );
    ...
    */
    ...
    private static function _minify_select() {
        ...
        return array(
            self::MINIFY_DISABLED  => esc_html__('Disabled', 'trulywp-cache'),
            self::MINIFY_HTML_ONLY => esc_html__('HTML', 'trulywp-cache'),
            self::MINIFY_HTML_JS   => esc_html__('HTML & Inline JS', 'trulywp-cache')
        );
    }
    ...
    */
    ...
    public static function requirements_check() {
        ...
        if ( version_compare($GLOBALS['wp_version'], TRULYWP_CE_MIN_WP.'alpha', '<') ) {
            show_message(
                sprintf(
                    '<div class="error"><p>%s</p></div>',
                    sprintf(
                        __('The <b>%s</b> is optimized for WordPress %s. Please disable the plugin or upgrade your WordPress installation (recommended).', 'trulywp-cache'),
                        'TrulyWP Cache',
                        TRULYWP_CE_MIN_WP
                    )
                )
        ...
        // permission check
        if ( file_exists( TRULYWP_CE_CACHE_DIR ) && !is_writable( TRULYWP_CE_CACHE_DIR ) ) {
            show_message(
                sprintf(
                    '<div class="error"><p>%s</p></div>',
                    sprintf(
                        __('The <b>%s</b> requires write permissions %s on %s. Please <a href="%s" target="_blank">change the permissions</a>.', 'trulywp-cache'),
                        'TrulyWP Cache',
                        '<code>755</code>',
        ...
        if ( defined('AUTOPTIMIZE_PLUGIN_DIR') && $options['minify_html'] && get_option('autoptimize_html', '') != '' ) {
            show_message(
                sprintf(
                    '<div class="error"><p>%s</p></div>',
                    sprintf(
                        __('The <b>%s</b> plugin is already active. Please disable minification in the <b>%s</b> settings.', 'trulywp-cache'),
                        'Autoptimize',
                        'TrulyWP Cache'
                    )
                )
    ...
    */
    ...
    public static function missing_trailing_slash() {
        if ( ($permalink_structure = get_option('permalink_structure')) &&
            ...
            TrulyWP_Cache_Enabler_Disk::delete_advcache_settings(array(
    ...
    */
    ...
    public static function validate_settings($data) {
        ...
        if ( $data['expires'] > 0 ){
            ...
            TrulyWP_Cache_Enabler_Disk::delete_advcache_settings(array("expires"));
        }
        ...
        if ( strlen($data["excl_regexp"]) > 0 ) {
            ...
            TrulyWP_Cache_Enabler_Disk::delete_advcache_settings(array("excl_regexp"));
        }
        ...
        if ( strlen($data["excl_cookies"]) > 0 ) {
            ...
            TrulyWP_Cache_Enabler_Disk::delete_advcache_settings(array("excl_cookies"));
        }
    ...
    public static function settings_page() {
        ...
        if ( !defined('WP_CACHE') || !WP_CACHE ) {
            echo sprintf(
                    ...
                    sprintf(
                        __("%s is not set in %s.", 'trulywp-cache'),
        ...
        <div class="wrap" id="cache-settings">
            <h2>
                <?php _e("Cache Enabler Settings", "trulywp-cache") ?>
            </h2>

           
            <p><?php $size=self::get_cache_size(); printf( __("Current cache size: <b>%s</b>", "trulywp-cache"), ( empty($size) ? esc_html__("Empty", "trulywp-cache") : size_format($size) ) ); ?></p>

            <form method="post" action="options.php">
                <?php settings_fields('trulywp-cache') ?>
                ...
                <table class="form-table">
                    <tr valign="top">
                        <th scope="row">
                            <?php _e("Cache Expiry", "trulywp-cache") ?>
                        </th>
                        <td>
                            <fieldset>
                                <label for="cache_expires">
                                    <input type="text" name="trulywp-cache[expires]" id="cache_expires" value="<?php echo esc_attr($options['expires']) ?>" />
                                    <p class="description"><?php _e("Cache expiry in hours. An expiry time of 0 means that the cache never expires.", "trulywp-cache"); ?></p>
                                </label>
                            </fieldset>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row">
                            <?php _e("Cache Behavior", "trulywp-cache") ?>
                        </th>
                        <td>
                            <fieldset>
                                <label for="cache_new_post">
                                    <input type="checkbox" name="trulywp-cache[new_post]" id="cache_new_post" value="1" <?php checked('1', $options['new_post']); ?> />
                                    <?php _e("Clear the complete cache if a new post has been published (instead of only the home page cache).", "trulywp-cache") ?>
                                </label>

                                <br />

                                <label for="cache_new_comment">
                                    <input type="checkbox" name="trulywp-cache[new_comment]" id="cache_new_comment" value="1" <?php checked('1', $options['new_comment']); ?> />
                                    <?php _e("Clear the complete cache if a new comment has been posted (instead of only the page specific cache).", "trulywp-cache") ?>
                                </label>

                                <br />

                                <label for="cache_compress">
                                    <input type="checkbox" name="trulywp-cache[compress]" id="cache_compress" value="1" <?php checked('1', $options['compress']); ?> />
                                    <?php _e("Pre-compression of cached pages. Needs to be disabled if the decoding fails in the web browser.", "trulywp-cache") ?>
                                </label>

                                <br />

                                <label for="cache_webp">
                                    <input type="checkbox" name="trulywp-cache[webp]" id="cache_webp" value="1" <?php checked('1', $options['webp']); ?> />
                                    <?php _e("Create an additional cached version for WebP image support. Convert your images to WebP with <a href=\"https://optimus.io/en/\" target=\"_blank\">Optimus</a>.", "trulywp-cache") ?>
                                </label>

                                <br />

                                <label for="cache_clear_on_upgrade">
                                    <input type="checkbox" name="trulywp-cache[clear_on_upgrade]" id="cache_clear_on_upgrade" value="1" <?php checked('1', $options['clear_on_upgrade']); ?> />
                                    <?php _e("Clear the complete cache if any plugin has been upgraded.", "trulywp-cache") ?>
                                </label>
                            </fieldset>
                        </td>
                    </tr>
                    ...
                    <tr valign="top">
                        <th scope="row">
                            <?php _e("Cache Exclusions", "trulywp-cache") ?>
                        </th>
                        <td>
                            <fieldset>
                                <label for="cache_excl_ids">
                                    <input type="text" name="trulywp-cache[excl_ids]" id="cache_excl_ids" value="<?php echo esc_attr($options['excl_ids']) ?>" />
                                    <p class="description">
                                        <?php echo sprintf(__("Post or Pages IDs separated by a %s that should not be cached.", "trulywp-cache"), "<code>,</code>"); ?>
                                    </p>
                                </label>
                                ...
                                <label for="cache_excl_regexp">
                                    <input type="text" name="trulywp-cache[excl_regexp]" id="cache_excl_regexp" value="<?php echo esc_attr($options['excl_regexp']) ?>" />
                                    <p class="description">
                                        <?php _e("Regexp matching page paths that should not be cached.", "trulywp-cache"); ?><br>
                                        <?php _e("Example:", "trulywp-cache"); ?> <code>/(^\/$|\/robot\/$|^\/2018\/.*\/test\/)/</code>
                                    </p>
                                </label>
                                ...
                                <label for="cache_excl_cookies">
                                    <input type="text" name="trulywp-cache[excl_cookies]" id="cache_excl_cookies" value="<?php echo esc_attr($options['excl_cookies']) ?>" />
                                    <p class="description">
                                        <?php _e("Regexp matching cookies that should cause the cache to be bypassed.", "trulywp-cache"); ?><br>
                                        <?php _e("Example:", "trulywp-cache"); ?> <code>/^(wp-postpass|wordpress_logged_in|comment_author|(woocommerce_items_in_cart|wp_woocommerce_session)_?)/</code><br>
                                        <?php _e("Default if unset:", "trulywp-cache"); ?> <code>/^(wp-postpass|wordpress_logged_in|comment_author)_/</code>
                                    </p>
                                </label>
                            </fieldset>
                        </td>
                    ...
                    <tr valign="top">
                        <th scope="row">
                            <?php _e("Cache Minification", "trulywp-cache") ?>
                        </th>
                        <td>
                            <label for="cache_minify_html">
                                <select name="trulywp-cache[minify_html]" id="cache_minify_html">
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <p class="description"><?php _e("Saving these settings will clear the complete cache.", "trulywp-cache") ?></p>
                        </td>
                    </tr>
                </table>
            </form>

...
$settings = _twp_read_settings($settings_file);
...
// generate cache path
function _twp_ce_file_path($path = NULL) {
    $path = sprintf(
        '%s%s%s%s',
        WP_CONTENT_DIR . '/cache/trulywp-cache',
        DIRECTORY_SEPARATOR,
        parse_url(
            'http://' .strtolower($_SERVER['HTTP_HOST']),
            PHP_URL_HOST
...
// read settings file
function _twp_read_settings($settings_file) {
